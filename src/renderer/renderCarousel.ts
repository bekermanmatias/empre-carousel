...
export type ImageMetricReport = {width:number;height:number;aspectRatio:number;templateFit:number}
...
export async function renderCarousel(carousel: Carousel, outputDirectory: string, fileName: (index:number, position?:number, template?:string) => string): Promise<RenderReport> {
  ...
  try {
    for (const [index, slide] of carousel.slides.entries()) {
      ...
      const measured=await page.evaluate(() => { const root=document.querySelector('.slide')!; const rect=root.getBoundingClientRect(); const warnings=[...document.querySelectorAll<HTMLElement>('.has-overflow')].map(node=>({field:node.dataset.overflowField ?? 'unknown',type:'overflow' as const})); const textMetrics=[...document.querySelectorAll<HTMLElement>('[data-text-field]')].map(node=>({field:node.dataset.textField ?? 'unknown',lineCount:Number(node.dataset.lineCount),scrollHeight:Number(node.dataset.scrollHeight),clientHeight:Number(node.dataset.clientHeight),overflow:node.dataset.overflow==='true',fontSizeUsed:Number(node.dataset.fontSizeUsed),autoFitApplied:node.dataset.autoFitApplied==='true',maxLines:node.dataset.maxLines ? Number(node.dataset.maxLines) : undefined})); const image=document.querySelector<HTMLImageElement>('.image-slot img'); const imageMetrics=image?.naturalWidth && image.naturalHeight ? {width:image.naturalWidth,height:image.naturalHeight,aspectRatio:Number((image.naturalWidth/image.naturalHeight).toFixed(4))} : undefined; return {warnings,textMetrics,imageMetrics,dimensions:{width:Math.round(rect.width),height:Math.round(rect.height)}} })
      ...
      const imageMetrics=measured.imageMetrics ? {...measured.imageMetrics,templateFit:fitForTemplate(measured.imageMetrics.aspectRatio,slide.template as TemplateName)} : undefined
