...
try { for (const fixture of contentFixtures) { const page=await browser.newPage({viewport:{width:1080,height:1350}}); await page.goto(`http://127.0.0.1:4174/?slide=${encodeURIComponent(JSON.stringify(fixture.slide))}`,{waitUntil:'networkidle'}); await page.evaluate(async()=>{await document.fonts.ready}); const measured=await page.evaluate(()=>({warnings:[...document.querySelectorAll<HTMLElement>('.has-overflow')].map(node=>node.dataset.overflowField ?? 'unknown'),autoFits:[...document.querySelectorAll<HTMLElement>('[data-overflow-status="fitted"]')].map(node=>({field:node.dataset.overflowField ?? 'unknown',originalFontSize:Number(node.dataset.originalFontSize),finalFontSize:Number(node.dataset.finalFontSize)}))})); results.push({template:fixture.template,kind:fixture.kind,expectedOverflow:fixture.expectsOverflow,expectedAutoFit:Boolean(fixture.expectsAutoFit),...measured}); await page.close() } } finally { await browser.close(); await new Promise<void>(ok=>server.close(()=>ok())); await vite.close() }
...
const mismatches=results.filter(result=>(result.warnings.length>0)!==result.expectedOverflow || (result.autoFits.length>0)!==result.expectedAutoFit)
